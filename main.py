...
class FaceMonitor:
    ...
    def _extract_embedding(self, face_rgb: np.ndarray) -> np.ndarray:
        # DeepFace expects a filesystem path; persist the cropped face temporarily.
        temp_path: Optional[Path] = None
        try:
            with tempfile.NamedTemporaryFile(suffix=".jpg", delete=False) as tmp_file:
                temp_path = Path(tmp_file.name)
            bgr_face = cv2.cvtColor(face_rgb, cv2.COLOR_RGB2BGR)
            cv2.imwrite(str(temp_path), bgr_face)
            ...
            if temp_path:
                try:
                    ...
                    LOGGER.warning("Failed to delete temporary face image %s", temp_path)
